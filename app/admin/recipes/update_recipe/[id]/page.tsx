...
const UpdateRecipe = () => {
  ...
  useEffect(() => {
    if (recipe?.recipe) {
      setNewRecipe({
        teaching_modality: recipe.recipe.teaching_modality || "",
        name: recipe.recipe.name || "",
        preparation_method: recipe.recipe.preparation_method || "",
        required_utensils: recipe.recipe.required_utensils || "",
        description_of_recipe: recipe.recipe.description_of_recipe || "",
        observations: recipe.recipe.observations || "",
        home_measurements: recipe.recipe.home_measurements,
        prep_time: recipe.recipe.prep_time || 0,
        timeOfCoccao: recipe.recipe.timeOfCoccao || 0,
      ...
      if (recipe.ingredients && Array.isArray(recipe.ingredients)) {
        const formattedIngredients = recipe.ingredients.map((ing) => ({
          ...
          cooked_weight: ing.cooked_weight,
          unit_of_measure: ing.unit_of_measure,
          homemade_measure: ing.homemade_measure,
        }));
  ...
  const { data: dataPost, loading: postLoading, error: postError, upDate: updateRecipe } = useUpdate<any>("recipes");

  //colocar form

  const updateNewRecipe = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      const responseData = await updateRecipe(recipe.recipe.id, newRecipe);
    ...
    <div className="flex w-full flex-col justify-start gap-4">
      ...
      <div className="flex flex-col-reverse md:flex-row w-full">
        <div className="flex flex-col w-full antialiased">
          <h3 className="font-bold text-xl md:text-2xl">Atualizar dados da Receita</h3>
